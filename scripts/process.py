    ...
    if ul_element:
        ...
        for li in li_elements:
            ...
            if a_tag:
                ...
                href_link = str(a_tag["href"])
                if "preparatory-documents" in href_link:
                    continue
        ...
        print(
            'The specified <ul> element with class "dividers" was not found on the page.'
        )
...
def parse_pdf(links, treaties):
    ...
    data = []
    for link in links:
        treaty_url = urljoin(BASE_URL, str(link))
        soup = BeautifulSoup(
            requests.get(treaty_url, timeout=60).content, "html.parser"
        )
        ...
        for i in range(len(ul_element)):
            ...
            if a_tag:
                ...
                if "ShowResults" in href_link:
                    pdf_links.append(urljoin(BASE_URL, str(href_link)))

    if not pdf_links:
        print(
            "No treaty results links discovered from current WIPO page structure; keeping existing dataset."
        )
        return

    for link, treaty in zip(pdf_links, treaties):
        soup1 = BeautifulSoup(requests.get(link, timeout=60).content, "html.parser")
    ...
    if not data:
        print("No treaty rows parsed from WIPO result pages; keeping existing dataset.")
        return
    df = pd.DataFrame(
        data,
        columns=[
